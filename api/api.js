...
module.exports.load = async function (app, db) {
  ...
  app.get("/api/v2/userinfo", async (req, res) => {
    ...
    if (await userinforeq.statusText == "Not Found") {
      console.log("App ― An error has occured while attempting to get a user's information");
  ...
  /**
   * GET /giftres
   * Gifts resources to another user.
   */
  ...
  async function check(req, res) {
    ...
    ejs.renderFile(
      ...
      function (err, str) {
        ...
        if (err) {
          console.log(`App ― An error has occured on path ${req._parsedUrl.pathname}:`);
          console.log(err);
